    ...
    vscode.workspace.saveAll().then(
      success => {
        if (success) {
          ...
          var openTextDoc;
          iRiteChannel.appendLine("Result: " + result.toString());
          if (result.length > 0) {
            {
              // if(result.length > 1){
              ...
              iRiteChannel.appendLine("Open Document fileName: " + openTextDoc.fileName);

              openTextDoc.then(srcFile => {
                iRiteChannel.show();
                iRiteChannel.appendLine("srcFile: " + srcFile.fileName);
                vscode.window.showTextDocument(srcFile).then(srcEditor => {
                  checkEngineDefined().then(engineType => {
                    checkEnginePathDefined(engineType.toString()).then(engineParameters => {
                      ...
                      iRiteChannel.appendLine("Building: " + srcFile.fileName);
                      iRiteChannel.appendLine("srcEditor: " + srcEditor.document.fileName);
